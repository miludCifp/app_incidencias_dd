...
async function cargarUsersEnTabla() {
    ...
    usuarios.forEach(usuario => {
        ...
        const row = `
            <tr>
                ...
                <td>${convertirRolToFrontend(usuario.rol)}</td>
    ...
    tabla.querySelectorAll('.btn-dar-de-baja').forEach(botonEliminar => {
        botonEliminar.addEventListener('click', function () {
            ...
            eliminarTrabajador(this, usuario);
    ...
    tabla.querySelectorAll('.btn-edit-user').forEach(botonEliminar => {
        botonEliminar.addEventListener('click', function () {
            ...
            const usuario = usuarios.find(user => user.idUsuario.toString() === idUsuario);
            // Llamar a la función eliminarTrabajador pasando el botón y el objeto de usuario
            cargarEditarUsuario(usuario, token);
...
function convertirRolToServidor(rol) {
    switch (rol) {
        case 'Admin':
...
function cargarEditarUsuario(objetoUsuario, token) {

    var idUser = objetoUsuario.idUsuario;

    if (idUser === obtenerIDUser(token)) {
        console.log('vamos a editar el ADMIN : ' + idUser);
        // Mostrar mensaje de confirmacion 
        // Confirmar si el usuario realmente quiere eliminar la incidencia
        Swal.fire({
            title: '¿Está seguro de que desea editar su cuenta actual?',
            text: 'Te redirigimos a la pagina "Mi perfil"',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                // Redirigir a la pagina Mi perfil
                window.location.href = 'ajustes_perfil_user';
            }
        });
    } else {
        console.log('vamos a editar el Trabajador : ' + idUser);
        ...
        console.log("Rol sacado del objeto usuario es >>> : " + rolUser);

        console.log("Rol convertido : " + convertirRolToFrontend(rolUser));

        console.log("Tipo de documento del user : " + TipoDocUser);
        ...
        document.getElementById("rolUserEdit").value = convertirRolToFrontend(rolUser);

        //document.getElementById("passwdUserEdit").value = passwdUser;
        ...
        document.getElementById("editarUsersForm").style.display = "block";


        // llamamos aqui a la funcion que edita el usuario y lo actualiza en la BD
        editarUserTrabajador(objetoUsuario);
...
function validarCamposTrabajador(tipoDoc, doc, tlfn, email) {
...
async function editarUserTrabajador(objetoUsuario) {
    ...
    console.log('el rol del usuario es : ' + rolUser);
    ...
    var rol = userOriginal.rol;


    // Escuchar el evento submit del formulario
    document.getElementById("editarUsersForm").addEventListener("submit", async function (event) {
        event.preventDefault();


        // Obtener los nuevos valores del formulario
        //var nuevaImagenPerfil = document.getElementById("imagenPerfil").value;
        var nuevoTipoDoc = document.getElementById("comboTipoDocEdit").value;
        var nuevoDoc = document.getElementById("inputDocEdit").value.trim();
        var nuevoGenero = document.getElementById("comboGeneroEdit").value;
        var nuevoNombre = document.getElementById("nombreUserEdit").value.trim();
        var nuevosApellidos = document.getElementById("apellidosUserEdit").value.trim();
        var nuevoTelefono = document.getElementById("telfUserEdit").value.trim();
        var nuevoEmail = document.getElementById("emailUserEdit").value.trim();
        var nuevaContrasena = document.getElementById("passwdUserEdit");
        var nuevoRol = convertirRolToServidor(document.getElementById("rolUserEdit").value);

        // Imprimir variables para depuracion
        console.log('Original Documento es ==>' + documento);
        console.log('Nuevo Documento es ==>' + nuevoDoc);
        console.log('Original Nombre es ==>' + nombre);
        console.log('Nuevo Nombre es ==>' + nuevoNombre);
        console.log('Original Apellidos es ==>' + apellidos);
        console.log('Nuevo Apellidos es ==>' + nuevosApellidos);
        console.log('Original TLFN es ==>' + telefono);
        console.log('Nuevo TLFN es ==>' + nuevoTelefono);
        console.log('Original Email es ==>' + email);
        console.log('Nuevo Email es ==>' + nuevoEmail);
        //console.log('Original Password es ==>' + password);
        //console.log('Nuevo Password es ==>' + nuevaContrasena);
        console.log('Original ROL es ==>' + rol);
        console.log('Nuevo ROL es ==>' + nuevoRol);


        // Verificar si ha habido cambios
        if (nuevoTipoDoc == tipoDoc &&
            nuevoDoc == documento &&
            nuevoGenero == genero &&
            nuevoNombre == nombre &&
        ...
        if (validarCamposTrabajador(nuevoTipoDoc, nuevoDoc, nuevoTelefono, nuevoEmail)) {
            ...
            if (nuevaContrasena != null) {
                ...
                if (txtNuevaContrasena != '') {
                    ...
                    if(validarContraseñaNueva(txtNuevaContrasena)){
                        ...
                        actualizarPasswdTrabajador(idUserOriginal, usuarioActualizado,token);
                    }
                }
            } else {
                console.warn("Editando usuario sin Contraseña --->", usuarioActualizado);
                actualizarUsuarioTrabajador(urlPut, usuarioActualizado,token);
            }
        }
...
function crearCampoNewContraseña(){
    ...
    btnCambiarPasswd.addEventListener("click", function () {
        ...
        // Ocultamos el botón Cambiar contraseña
...
async function actualizarUsuarioTrabajador(urlPut, objetoUsuario,token) {
...
async function eliminarTrabajador(boton, objetoUsuario) {
    ...
    // Obtenemos el rol del usuario trabajador desde la tabla
    var rolUser = objetoUsuario.rol;
    console.log("El rol de user sacado del objeto es --->" + rolUser);
    ...
    if (idUser === obtenerIDUser(token)) {
        console.log('vamos a borrar el ADMIN : ' + idUser);
        // Mostrar mensaje de confirmacion 
        // Confirmar si el usuario realmente quiere eliminar la incidencia
        Swal.fire({
        ...
        console.log('vamos a borrar UN Trabajador : ' + idUser);
        // Confirmar si el usuario realmente quiere eliminar la incidencia
        Swal.fire({
...
function volverListaUsuarios() {
    ...
    document.getElementById("editarUsersForm").style.display = "none";
